...
def main(args):
    if args['dataset'] == 'NUPT-Metro':
        train_file = '/home/xsh/XSH/MADNet-master/npydata/nupt_train.npy'
...
class SSIM(nn.Module):
    def __init__(self, window_size=11):
        ...
        self.gaussian_window = self._get_gaussian_window(window_size)

    def _get_gaussian_window(self, window_size, sigma=1.5):
        x = np.linspace(-sigma, sigma, window_size)
    ...
    def forward(self, img1, img2):
        ...
        mu1 = F.conv2d(img1, self.gaussian_window, padding=self.window_size//2, groups=1)
        mu2 = F.conv2d(img2, self.gaussian_window, padding=self.window_size//2, groups=1)

        mu1_sq = mu1.pow(2)
        mu2_sq = mu2.pow(2)
        mu1_mu2 = mu1 * mu2

        sigma1_sq = F.conv2d(img1 * img1, self.gaussian_window, padding=self.window_size//2, groups=1) - mu1_sq
        sigma2_sq = F.conv2d(img2 * img2, self.gaussian_window, padding=self.window_size//2, groups=1) - mu2_sq
        sigma12 = F.conv2d(img1 * img2, self.gaussian_window, padding=self.window_size//2, groups=1) - mu1_mu2
...
class GradientLoss(nn.Module):
    ...
    def forward(self, input, target):
        def get_gradient(input):
            grad_x = input[:, :, :, 2:] - input[:, :, :, :-2]
            grad_y = input[:, :, 2:, :] - input[:, :, :-2, :]
            grad_x = F.pad(grad_x, (0, 2, 0, 0), mode='replicate')  
            grad_y = F.pad(grad_y, (0, 0, 0, 2), mode='replicate') 
            return grad_x, grad_y
        
        pred_grad_x, pred_grad_y = get_gradient(input)
        gt_grad_x, gt_grad_y = get_gradient(target)
        
        grad_loss = torch.mean(torch.abs(pred_grad_x - gt_grad_x) + torch.abs(pred_grad_y - gt_grad_y))
        return grad_loss
...
def train(Pre_data, model, criterion, optimizer, epoch, args):
    ...
    criterion_ssim = SSIM(window_size=11).cuda() 
    criterion_gradient = GradientLoss().cuda()    
    ...
    for i, (fname, img, fidt_map, kpoint) in enumerate(train_loader):
        ...
        loss_mse = criterion_mse(d6, fidt_map)
        loss_ssim = 1 - criterion_ssim(d6, fidt_map)  
        loss_gradient = criterion_gradient(d6, fidt_map)
        
        loss = (0.5 * loss_mse + 0.3 * loss_ssim + 0.2 * loss_gradient)
        ...
        if i % args['print_freq'] == 0:
            print('Epoch: [{0}][{1}/{2}]'.format(epoch, i, len(train_loader)) +
                  '\tTime {0:.4f} ({1:.4f})'.format(batch_time.val, batch_time.avg) +
                  '\tData {0:.4f} ({1:.4f})'.format(data_time.val, data_time.avg) +
                  '\tLoss {0:.4f} ({1:.4f})'.format(losses.val, losses.avg) +
                  '\tMSE Loss {0:.4f}'.format(loss_mse.item()) +
                  '\tSSIM Loss {0:.4f}'.format(loss_ssim.item()) +
                  '\tGradient Loss {0:.4f}'.format(loss_gradient.item()))
...
def validate(Pre_data, model, args): 
    ...
    '''output coordinates'''
    f_loc = open("./local_eval/nupt_localization.txt", "w+")
...
def LMDS_counting(input, w_fname, f_loc, args):
    ...
    # input[input < 135.0 / 255.0 * input_max] = 0
    input[input < 120.0 / 255.0 * input_max] = 0
    input[input > 0] = 1
